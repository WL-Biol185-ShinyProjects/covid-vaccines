...
function(input, output, session) {
  ...
  output$percapita <- renderPlot({ 
    ...
    pop_hopkins_fully <- left_join(hopkins_fully,
                                   country_pop, 
    ...
    ggplot(pop_hopkins_fully, 
           aes(reorder(Country_Region,
                       fully_per_capita),
                       fully_per_capita,
                       fill = World.Share)) + 
  ...
  output$globalvariants <- renderPlot ({ 
    ...
    ggplot(variants_popular, aes(month_write, n, fill = variant)) +
      ...
      coord_flip() +
      theme_classic() +
      labs(title = "Covid-19 Variant Prevelance By Year", 
               y = "Relevant Prevelance", 
               x = "Month", 
            fill = "Variant")
                                      })
  ...
  output$socioeconomicsbox <- renderPlot({
    ...
    ggplot(data = pop_hopkins_fully, aes(x = fully_per_capita, 
                                         y = Country_Region, 
                                      fill = GDP.nominal.per.capita)) +
      geom_tile()+
      theme(axis.text.x = element_text(angle = 90, 
                                       vjust = 0.5, 
                                       hjust = 1)) +
      labs(title = "GDP and Vaccination Rates In the World", 
           subtitle = "Top 10 and Bottom 10 GDP countries chosen",
                  x = "Percent Fully Vaccinated Per Country", 
                  y = "Country", 
               fill = "Nominal GDP Per Capita")
  ...
  output$PercentVaccinatedHeatMap <- renderLeaflet({
    ...
    hopkins_pop <- left_join(hopkins, 
                             population_by_country, 
                             by = c("Country_Region" = "Country..or.dependency.")) 
    

    hopkins_pop$percent_partial <- 
      (hopkins_pop$People_partially_vaccinated / hopkins_pop$Population..2020.)*100
    
    hopkins_pop$percent_fully <- 
      (hopkins_pop$People_fully_vaccinated / hopkins_pop$Population..2020.)*100 
    ...
    leaflet(world_geo) %>%
      ...
      addLegend(     pal = mypalette, 
                  values =~ People_fully, 
                 opacity = 0.9, 
                   title = "Percent fully vaccinated by country", 
                position = "bottomleft" )
  ...
  output$PartiallyVaccinatedHeatmap <-  renderLeaflet({
      ...
      leaflet(world_geo) %>%
        ...
        addPolygons(popup = partial_popup,
             ...
             labelOptions = labelOptions(style = list("font-weight" = "normal", 
                                     ...
                                     direction = "auto")) %>%
        addLegend( pal = mypalette, 
                values =~ percent_partial, 
               opacity = 0.9, 
                 title = "Percent partially vaccinated by country", 
              position = "bottomleft" )
